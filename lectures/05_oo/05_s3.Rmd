...
- Use `sloop::ftype()` to see if a function is generic. If it has the word "generic" anywhere, it can be used as an S3 generic.
...
- If you don't know, whenever you just run something and have it print to the console, that is R implicitly running `print()`. So this looks better too:
    ```{r}
    x
    ```
