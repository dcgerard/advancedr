...
- The slots are
  ...
  - `@samples`: A character vector with the sample names.
  - `@header`: A `DataFrameList` with the following elements:
      - `fixed`: An optional `DataFrameList` with elements
          - `QUAL`: A `DataFrame` with a single column called `Description`.
          - `FILTER`: A `DataFrame` with a single column called `Description`.
          - `ALT`: A `DataFrame` with a single column called `Description`. 
          - `REF`: A `DataFrame` with a single column called `Description`.
      - `info`: A `DataFrame` with `INFO` information. Must have three columns `Number`, `Type`, and `Description`.
      - `geno`: A `DataFrame` of `FORMAT` information. This has three columns, `Number`, `Type`, and `Description`.
