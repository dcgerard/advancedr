...
- Whenever a test generates something randomly, I like to set a seed for reproducibility.
    - A [random seed](https://en.wikipedia.org/wiki/Random_seed) initalizes the pseudorandom process. So any "random draws" in R will be the same if you set the same seed via `set.seed()`.
