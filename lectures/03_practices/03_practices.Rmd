...
- Most of the interesting applications of R computing come from Statistics. 

- For this lecture, I will use an example from statistical genetics (because that is my field).

- Suppose I am making a method to test for [Hardy-Weinberg equilibrium](https://en.wikipedia.org/wiki/Hardy%E2%80%93Weinberg_principle) (HWE). 

- In genetics there are often two "alleles" A and a. Recall that humans have two copies of their genome, so some individuals have AA, some have Aa, and some have aa. If $Pr(A) = p$, then HWE states that
    \begin{align}
    Pr(AA) &= p^2\\
    Pr(Aa) &= 2p(1-p)\\
    Pr(aa) &= (1-p)^2
    \end{align}
    If HWE is not fulfilled, then $Pr(AA) \neq p^2$ or $Pr(Aa) \neq 2p(1-p)$ or $Pr(aa) \neq (1-p)^2$.
  
- Let $n_{AA}, n_{Aa}, n_{aa}$ be the number of individuals observed to have AA, Aa, and aa, respectively. Let $n = n_{AA} + n_{Aa} + n_{aa}$.

- A goal is often to test if HWE is satisfied. That is, do $n_{AA}/n, n_{Aa}/n, n_{aa}/n$ follow the frequencies expected under HWE?
...
- We then compare $n_{AA}/n$ to $\hat{p}^2$, we compare $n_{Aa}/n$ to $2\hat{p}(1-\hat{p})$, and we compare $n_{aa}/n$ to $(1-\hat{p})^2$.

- The two canonical ways to do this are via a $\chi^2$ test or a likelihood ratio test.

- For the $chi^2$ test we calculate
    $$
    X = \frac{(n_{AA} - n\hat{p}^2)^2}{n\hat{p}^2} + \frac{(n_{Aa} - n\hat{p}(1-\hat{p}))^2}{n\hat{p}(1-\hat{p})} + \frac{(n_{aa} - n(1-\hat{p})^2)^2}{n(1-\hat{p})^2}
    $$

- If the null were true, then $X \sim \chi^2_1$, so the $p$-value is
    ```{r, eval = FALSE}
...
- If you just don't like the bahavior, but don't want to kill the program, use `warning()`.
