...
- If you try to optimize too soon, you will usually end up wasting more time that you save. Your code will usually become clunky and harder to maintain.
...
- You cannot profile C/C++ code using `{profvis}`. You would have to use a C++ profiler like [gperftools](https://github.com/gperftools/gperftools).
...
For summing up the columns of a matrix, I can think of at least three ways

1. `colSums()`
2. Matrix multiplication on the left by a vector of `1`'s
3. Using a for-loop on the columns, calculating the sum each time.

Create functions that implements all three of these approaches and tell me which one is the fastest for $100\times 100$, $1000\times 100$, and $100\times 1000$ matrices.
...
## For loops are always the worst
