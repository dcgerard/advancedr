---
title: "Data 496/696 Style Guide"
author: "David Gerard"
date: "`r Sys.Date()`"
output:  
  html_document:
...
- Each organization has a style guide that you should adhere to.
...
- An [**infix**](https://en.wikipedia.org/wiki/Infix_notation) is one where arguments on both sides of it are used in a function. The alternative is [**prefix**](https://en.wikipedia.org/wiki/Polish_notation) notation. Compare
    ```{r}
    5 + 10 ## infix notation
...
- In R, the last value evaluated in a function will be implicitely returned. I think this is bad practice since it makes it harder to reason about what R is returning. So always include a `return()` statement. **Never** do
    ```{r, eval = FALSE}
