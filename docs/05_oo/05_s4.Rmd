...
- In S4, you need to explicitly run the `methods::setClass()` function to define the class. This makes the structure explicitly enforced.
    ...
    - The `prototype` argument contains a list of default values for each slot. You should always include a prototype.
...
- Above, `.Generic` is a variable defined inside the evaluation environment of the `setMethod()` definition.
...
- All S4 methods are required to be documented.
