...
- E.g.: To calculate the column means in S3 OOP, we would probably create a generic function for column means.
    ...
    col_means.data.frame <- function(x) {
      mean_vec <- rep(NA_real_, length.out = length(x))
      names(mean_vec) <- names(x)
...
- A function for a specific class is a **method**. Like the `col_means()` method for our R6 class above.
...
- `class()` will return the result of `typeof()` if an object has no `class` attribute (so it is not the most useful for determining class).
